    <div>
        ...
        <div class="row">
            <div class="col-lg-8">
                <h1>Data Surat yang belum di disposisi</h1>
            </div>
        </div><!--/.row-->
...
<div class="panel-body">
    <table class="table">
                         ...
                         <tbody>
                             <?php
  $qjumlah=mysqli_query($conn,"SELECT tb_user. * , tb_surat. * FROM tb_surat INNER JOIN tb_user ON tb_user.id_user = tb_surat.id_user where id_tipe_srt='T001' AND NOT EXISTS (SELECT * FROM tb_disposisi WHERE tb_surat.id_srt=tb_disposisi.id_srt)");
  ...
  ?>
    ...
    //paging
    $no=1+$posisi;
                            $surat_klr = $conn->query("SELECT tb_user. * , tb_surat. * FROM tb_surat INNER JOIN tb_user ON tb_user.id_user = tb_surat.id_user where id_tipe_srt='T001' AND NOT EXISTS(SELECT * FROM tb_disposisi WHERE tb_surat.id_srt=tb_disposisi.id_srt) limit $posisi,$batas  ");
  ...
  for($i=1; $i<=$hal; $i++){
  ?>
  <li <?php if($page==$i){ echo "class='active'";}?>><a href="?page=dispo&act=belum&hal=<?php echo $i;?>"><?php echo $i;?></a></li>
    <?php
  }
  ?>
  </ul>
